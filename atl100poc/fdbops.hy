...
;;; first take fdb class in hy
;;; Probably not a complete set of FaunaDB ops.
...
(defclass FdbOps [object]
  ...
  (defn update-data [self ref key value]
    (self._debug "update-feedher: begin")
    (if-not self.client
            (self.client_factory))
    (setv rt None)
    (setv datadct {})
    ...
    (try
     ...
     (except [e [NotFound]]
       (print "warning: update-feedher " % e))
     (except [e [UnexpectedError]]
       (print "error: update-feedher " % e)
       (sys.exit)))
    (self._debug "update-feedher: end")
    rt)
  ...
  (defn get_by_tag [self idx tagstr size]
